...
ProtoBot is a Python 3.12+ Minecraft protocol client. The package lives in `protobot/` at the repository root: `client.py` and `cli.py` provide the client and diagnostic entry points, while `state.py`, `world.py`, `navigation.py`, `modlist.py`, and `events.py` contain higher-level behavior. `cli_app.py` is the unified bot CLI (`protobot login|run|plugins|setup`); `plugin.py` is the plugin framework (discovery, dependency ordering, hot load/reload/close); `session.py` holds `BotSession`/`BotContainer`; `config.py` is a zero-dependency YAML-subset codec for `config.yaml`; `text.py` renders chat components as plain text; `tui.py` is the optional Textual full-screen UI (guarded import — falls back to plain logging without the `tui` extra). Low-level wire handling is grouped in `protobot/protocol/` (`codec.py`, framing, connection, NBT, and version tables). Movement and collision code belongs in `protobot/physics/`. Versioned block data is stored as compressed JSON under `protobot/data/`. Example plugins live in `plugins/` at the repository root. Tests live in `tests/` at the repository root, outside the installed package. `run_bot.py` is a top-level convenience shim (PyCharm right-click → `protobot run`) and is deliberately excluded from the distribution.
